...
from flask_babel import Babel  # Updated to use Flask-Babel
import os
...
def create_user():
    ...
    if not user_datastore.find_user(email="test@example.com"):
        user_datastore.create_user(username="testuser", email="test@example.com", password=generate_password_hash("password"))
    db.session.commit()
